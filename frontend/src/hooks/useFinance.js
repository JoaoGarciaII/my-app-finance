...
export function useSummary() {
  ...
  const fetch = useCallback(async () => {
    setLoading(true)
    try { const r = await api.get('/summary'); setData(r.data) } catch {}
    setLoading(false)
  }, [])
  useEffect(() => { fetch() }, [])
...
export function useTransactions() {
  ...
  const fetch = useCallback(async () => {
    setLoading(true)
    try { const r = await api.get('/transactions'); setTransactions(r.data) } catch {}
    setLoading(false)
  }, [])
  useEffect(() => { fetch() }, [])
  ...
  return { transactions, loading, refresh: fetch, add, remove }
}
...
export function useSubscriptions() {
  ...
  const fetch = useCallback(async () => {
    setLoading(true)
    try { const r = await api.get('/subscriptions'); setSubscriptions(r.data) } catch {}
    setLoading(false)
  }, [])
  useEffect(() => { fetch() }, [])
  const add = async (p) => { await api.post('/subscriptions', p); fetch() }
  const remove = async (id) => { await api.delete(`/subscriptions/${id}`); fetch() }
  return { subscriptions, loading, refresh: fetch, add, remove }
}
...
export function useBills() {
  ...
  const fetch = useCallback(async () => {
    setLoading(true)
    try { const r = await api.get('/bills', { params: { month } }); setBills(r.data) } catch {}
    setLoading(false)
  }, [])
  useEffect(() => { fetch() }, [])
  ...
  return { bills, loading, refresh: fetch, add, toggle, remove }
}
...
export function useGoals() {
  ...
  return { goals, loading, refresh: fetch, add, deposit, remove }
}
